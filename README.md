...
-   **Бесшовный вход в Mini App:** Мгновенная авторизация и открытие Mini App внутри Telegram.
...
* **Backend & CI/CD**: Yandex.Cloud Functions (Node.js) & YC CLI
* **AI/LLM**: Google Gemini, Workers AI, Stability AI, Kie.ai, FusionBrain, DeepSeek
- Cloudflare Workers AI — бесплатные T2I/I2I.
- Kie.ai — основной платный провайдер (T2I, I2V, V2V, A2V, TTS).
- Gemini — Основной чат, распознавание и Vision.
- Bothub / Stability / FusionBrain — резервные сервисы для моделей.
...
* **Хранилище данных**: Yandex Database (YDB) для медиа, промптов, настроек и логов.
...
2.  **Установите зависимости:**
    ...
    npm install
    ```
3.  **Настройте `function-spec.yaml`:**
    Откройте файл `function-spec.yaml` и замените плейсхолдеры `YOUR_...` вашими реальными токенами и ID.

### 3. Настройка Переменных Окружения (Секретов и Привязок)

Вам необходимо настроить **Секреты** (API-ключи) и **Привязки** в GitHub.
...
| *И другие `[vars]` из секции `[environment]` в `deploy.yml`* | | |
...
| *И все остальные ключи `[sesrets]` из секции `[environment]` в `deploy.yml`* | |

### 4. Установка вебхука

После получения URL API-шлюза, установите вебхук, чтобы Telegram знал, куда отправлять обновления:
